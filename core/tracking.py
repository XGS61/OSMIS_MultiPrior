...
class image_saver():
    ...
    def save(self, fake, epoch, i=None):
        ...
        if not self.no_masks:
            ...
            if "pred_masks" in fake:
                predicted_masks = self.paint_mask(fake["pred_masks"])
                torchvision.utils.save_image(
                    predicted_masks,
                    os.path.join(self.folder_images, epoch + "_pred_mask" + self.ext),
                )
            if self.phase == "test":
                self.save_label_masks(fake["masks"], epoch)
                if "pred_masks" in fake:
                    self.save_label_masks(fake["pred_masks"], epoch + "_pred")

    def save_label_masks(self, masks, epoch):
        labels = torch.argmax(masks, dim=1).detach().cpu().numpy().astype(np.uint8)
        for index, label in enumerate(labels):
            suffix = "" if len(labels) == 1 else f"_{index}"
            path = os.path.join(
                self.folder_images,
